...
public class SymSpell extends QuerySuggestion
{
    ...
    public void CreateDictionary(String corpus, String language) throws FileNotFoundException
    {
        ...
        System.out.println("\rDictionary: " + wordCount + " words, " + dictionary.size() + " entries, edit distance=" + MAXDISTANCE + " in " + (endTime-startTime)+"ms ");
    }
    ...
    @Override
    public HashSet<String> Edits(String word, int editDistance, HashSet<String> result)
    {
        editDistance++;
        if (word.length() > 1)
        {
            for (int i = 0; i < word.length(); i++)
            {
                ...
                if (result.add(delete))
                {
                    //recursion, if maximum edit distance not yet reached
                    if (editDistance < MAXDISTANCE) Edits(delete, editDistance, result);
                }
            }
        }
        return result;
...
//        else return suggestions;
    ...
    @Override
    public List<List<suggestItem>> Correct(String[] query, String language)
    {
        List<List<suggestItem>> suggestions = new ArrayList<>();
        String word = null;
        /*            
        ...
        */
        ...
        for (int i = 0; i < query.length; i++) {
            ...
            suggestions.add(Lookup(word, language, MAXDISTANCE));
        }
